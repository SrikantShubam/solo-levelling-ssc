...
def merge_grok_results_into_pdf(pdf_name, grok_page_results):
    ...
    for q in data.get("questions", []):
        ...
        if page not in grok_page_results:
            continue
        grok_qs = grok_page_results[page]
        grok_q = None
        for gq in grok_qs:
...
# Test with the data we already have
import ast
# Read the Grok results from merge_grok2.py format
grok_data = {
...
result = merge_grok_results_into_pdf("2020_tier1_prepp_shift1", grok_data)
print(f"Test: {result}")
